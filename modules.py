...
def bottle_neck(filters, div=4, kernel=3):
    ...
    def main(x):
        ...
        for i, feature in enumerate(features):
            x = conv(div_channel, kernel, dilation_rate=dilation_rate[i], padding='same')(feature)
            x = conv(div_channel, kernel, dilation_rate=dilation_rate[i], padding='same', groups=div_channel)(x)
            if concat_list:
