...
def tmp(n_class, base_filters=64, depth=3):
    def main(inputs):
        ...
        ### Encoder
        for i in range(depth):
            x, skip = encoder.base_2(filters[i])(x)
            skip = skip_connection.base_2(filters[i], pool=None)(skip)
            skip_conn_list.append(skip)
        ### BottleNeck
        x = latent.base_2(filters[-1])(x)
        ### Decoder
        for i in reversed(range(depth)):
            x = decoder.base_2(filters[i])(x, skip_conn_list[i])
